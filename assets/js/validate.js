...
nameContent.addEventListener('blur', (event) => {
  if (nameContent.value === '') {
    ...
    errorName.textContent = 'Enter alphabet characters and spaces only!'
  }
})
...
dateContent.addEventListener('blur', (event) => {
  if (isValidStartDate(dateContent.value)) {
    ...
    errorDate.textContent = 'The start date cannot be from today and past!'
  }
})
...
expContent.addEventListener('blur', (event) => {
  if (expContent.value === '') {
    ...
    errorExp.textContent = 'The experience field cannot be empty!'
  }
})
